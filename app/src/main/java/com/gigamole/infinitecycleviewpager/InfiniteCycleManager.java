...
class InfiniteCycleManager implements OnNotifyDataSetChangedListener {
	...
	public InfiniteCycleManager(final Context context, final ViewPageable viewPageable,
	...
	public void processAttributeSet(final AttributeSet attributeSet) {
		if (attributeSet == null)
			return;
		final TypedArray typedArray = mContext.obtainStyledAttributes(attributeSet,
				mIsVertical ? R.styleable.VerticalInfiniteCycleViewPager
						: R.styleable.HorizontalInfiniteCycleViewPager);
		try {
			setMinPageScaleOffset(typedArray.getDimension(
			...
			setCenterPageScaleOffset(typedArray.getDimension(
			...
			setMaxPageScale(
					typedArray.getFloat(
							...
							DEFAULT_MAX_SCALE));
			setMediumScaled(typedArray.getBoolean(
			...
			setScrollDuration(typedArray.getInteger(
			...
			try {
				final int interpolatorId = typedArray
						.getResourceId(mIsVertical ? R.styleable.VerticalInfiniteCycleViewPager_icvp_interpolator
	...
	public boolean onTouchEvent(final MotionEvent event) {
		if (mViewPageable.getAdapter() == null || mViewPageable.getAdapter().getCount() == 0)
			return false;
		if (mIsAutoScroll || mIsInitialItem || mViewPageable.isFakeDragging())
			return false;
	...
	public int setCurrentItem(final int item) {
		...
		if (mViewPageable.getAdapter() == null || mViewPageable.getAdapter().getCount() < MIN_CYCLE_COUNT)
			return item;
		...
		if (mIsAdapterInitialPosition) {
			...
			return mViewPageable.getCurrentItem() + Math.min(count, item) - getRealItem();
	}
	...
	private void enableHardwareLayer(final View v) {
		final int layerType = Build.VERSION.SDK_INT > Build.VERSION_CODES.KITKAT ? View.LAYER_TYPE_NONE
	...
	private void checkHitRect(final MotionEvent event) {
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			mHitRect.set(mCastViewPageable.getLeft(), mCastViewPageable.getTop(), mCastViewPageable.getRight(),
					mCastViewPageable.getBottom());
		} else if (event.getAction() == MotionEvent.ACTION_MOVE
				&& !mHitRect.contains(mCastViewPageable.getLeft() + (int) event.getX(),
	...
	private void resetScroller() {
		...
		try {
			...
			final InfiniteCycleScroller infiniteCycleScroller = new InfiniteCycleScroller(mContext, mInterpolator);
	...
	public void startAutoScroll(final boolean isAutoScrollPositive) {
		if (mIsAutoScroll && isAutoScrollPositive == mIsAutoScrollPositive)
			return;
		mIsAutoScroll = true;
	...
	protected class InfiniteCyclePageTransformer implements PageTransformer {
		...
		public void transformPage(final View page, final float position) {
			...
			if (mItemCount == MIN_CYCLE_COUNT) {
				if (position > 2.0F || position < -2.0F || (mStackCount != 0 && position > 1.0F)
			...
			final float pageSubScaleEdgeOffset = (pageSize - (pageSize * (mMinPageScale + mCenterScaleBy))) * 0.5F;
			...
			// Detect when the count > 3 and pages at the center of bottom
				...
				scale = mMinPageScale;
				translation = (pageSize * 2.0F)
						- ((pageSize + pageMinScaleEdgeOffset - mMinPageScaleOffset) * fraction);
			...
			// Transform from minimum scale to medium scale or max
				...
				if (mIsMediumScaled) {
					final float startOffset = pageSize - pageSubScaleByOffset - pageMaxScaleEdgeOffset
							+ mMinPageScaleOffset;

					scale = (mMinPageScale + mCenterScaleBy) - (mCenterScaleBy * negativeFraction);
					translation = startOffset
							- ((startOffset - pageSubScaleEdgeOffset + mCenterPageScaleOffset) * positiveFraction);
				} else {
					final float startOffset = pageSize - pageMinScaleEdgeOffset + mMinPageScaleOffset;

					scale = (mMaxPageScale) - ((mMaxPageScale - mMinPageScale) * negativeFraction);
					translation = (startOffset)
							- ((startOffset - pageMaxScaleEdgeOffset + mCenterPageScaleOffset) * positiveFraction);
				}
			}
			...
			// Transform from center or max to min
				...
				if (mIsMediumScaled) {
					...
					translation = (-pageSubScaleEdgeOffset + mCenterPageScaleOffset)
							+ ((-pageSize + pageSubScaleByOffset + pageMaxScaleEdgeOffset + pageSubScaleEdgeOffset
					...
					translation = (-pageMaxScaleEdgeOffset + mCenterPageScaleOffset)
							+ ((-pageSize + pageMaxScaleEdgeOffset + pageMinScaleEdgeOffset - mMinPageScaleOffset
									- mCenterPageScaleOffset) * negativeFraction);
				}
			}
			// Detect when the count > 3 and pages at the center of bottom
			...
			// Translate page
			if (mIsVertical)
				ViewCompat.setTranslationY(page, translation);
			else
				ViewCompat.setTranslationX(page, translation);

			boolean needBringToFront = false;
			if (mItemCount == MIN_CYCLE_COUNT - 1)
			...
			case GOING_LEFT:
				...
				if (mInnerPageScrolledState == PageScrolledState.GOING_LEFT) {
					...
					if (mPageScrolledPositionOffset < 0.5F && position > -0.5F && position <= 0.0F)
						needBringToFront = true;
				}
				break;
			case GOING_RIGHT:
				...
				if (mInnerPageScrolledState == PageScrolledState.GOING_RIGHT) {
					...
					if (mPageScrolledPositionOffset > 0.5F && position >= 0.0F && position < 0.5F)
						needBringToFront = true;
				}
				break;
			default:
		...
		public void onPageScrolled(final int position, final float positionOffset, final int positionOffsetPixels) {
			...
			if (mState != ViewPager.SCROLL_STATE_SETTLING || mIsInitialItem) {
				...
				if (mOuterPageScrolledState == PageScrolledState.IDLE && positionOffset > 0) {
					...
					mOuterPageScrolledState = position == mPageScrolledPosition ? PageScrolledState.GOING_LEFT
			...
			if (mPageScrolledPositionOffset <= positionOffset)
				...
				mInnerPageScrolledState = PageScrolledState.GOING_RIGHT;
